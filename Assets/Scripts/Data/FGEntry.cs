...
public class FGEntry
{
    ...
    public FGEntry(string entry)
    {
        ...
        var dateFormatted = FGUtils.FormatString(split[0], FGUtils.DATE_WHITELIST);
        var temp = FGUtils.TryParseDateTime(dateFormatted, DateTime.Today, out var failed);
        if (!failed) Date = temp;
        
        var descriptionFormatted = FGUtils.FormatString(split[1], FGUtils.DESCRIPTION_WHITELIST);
        Description = descriptionFormatted;
        
        if (split.Count == 7)
        {
            var valueFormatted = FGUtils.FormatString(split[2], FGUtils.VALUE_WHITELIST);
            ...
            var categoryFormatted = FGUtils.FormatString(split[4], FGUtils.DESCRIPTION_WHITELIST);
            Category = categoryFormatted;
        
            var noteFormatted = FGUtils.FormatString(split[5], FGUtils.DESCRIPTION_WHITELIST);
            Note = noteFormatted;
        
            var ignoreFormatted = FGUtils.FormatString(split[6], FGUtils.BOOL_WHITELIST);
        ...
        else
        {
            ...
            {
                var valueFormatted = FGUtils.FormatString(split[3], FGUtils.VALUE_WHITELIST);
                if (float.TryParse(valueFormatted, out float outValue)) Value = outValue;
